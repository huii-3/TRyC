...
syms R1 R2 R3 R4 Vin real

%Req entre nodo X y GND
R234 = (R3*(R2+R4))/(R3+R2+R4)
...
%Tension Vout
Vout = simplify(Vx * R4/(R4 + R2)) %ahora tengo Vout en funcion de Vin

%divido a ambos lados por Vin
V_rel = Vout/Vin
...
%reemplazo valores en funcion simbolica
V_rel_num = subs(V_rel, {R1, R2, R3, R4}, {R1_v, R2_v, R3_v, R4_v})
%para tener el resultado flotante
V_rel_num = double(V_rel_num)
